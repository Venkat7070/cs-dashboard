export type Tier = "Enterprise" | "Platinum" | "Gold" | "Silver";
...
export interface AccountRaw {
  ...
  aoOwner: string;
  fdePod: string;
  /** Platform + Voice ARR — the dashboard's primary ARR figure. */
  arr: number;
  /** Total ARR (Platform + Voice + CPaaS), shown alongside but not used in KPI/health calcs. */
  totalArr: number;
