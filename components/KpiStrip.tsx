...
export default function KpiStrip({ kpi }: { kpi: KpiSummary }) {
  return (
    <div className="flex flex-wrap gap-3" role="region" aria-label="Portfolio KPIs">
      <Tile label="Platform + Voice ARR" value={formatCurrency(kpi.bookArr)} sub={formatCurrencyFull(kpi.bookArr)} />
      <Tile label="Total ARR" value={formatCurrency(kpi.totalArr)} sub={formatCurrencyFull(kpi.totalArr)} />
      <Tile
        label="ARR at risk"
